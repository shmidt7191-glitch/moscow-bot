...
5. Указывай на какой станции метро начать маршрут и какой выход.
6. Учитывай сезон и погоду.
7. Для маршрутов на полдня и целый день — добавляй 2-3 конкретных места где поесть или выпить кофе по пути (с адресом и ссылкой на Яндекс.Карты).
8. Для маршрутов после работы — только пешеходный путь без заведений.
9. Ссылки на Яндекс.Карты для ключевых точек маршрута: [название](https://yandex.ru/maps/?text=НАЗВАНИЕ+Москва)"""
...
ROUTE_PROMPTS = {
    "route_short": "маршрут после работы на 1-2 часа — приятная пешая прогулка по красивым улицам или бульварам, без заведений",
    "route_half": "маршрут на полдня (3-4 часа) — пешеходный маршрут по интересным местам + 2-3 места где поесть или выпить кофе по пути",
    "route_full": "маршрут на целый день — насыщенный пешеходный маршрут по Москве + несколько мест где поесть по пути",
}
...
def route_duration_keyboard():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🌆 После работы (1-2 часа)", callback_data="route_short")],
        [InlineKeyboardButton("🌅 Полдня (3-4 часа)", callback_data="route_half")],
        [InlineKeyboardButton("🌞 Целый день", callback_data="route_full")],
        [InlineKeyboardButton("◀️ Назад", callback_data="back_to_mood")],
    ])
...
async def handle_mood(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "mood_routes":
        await query.edit_message_text("На сколько времени маршрут?", reply_markup=route_duration_keyboard())
